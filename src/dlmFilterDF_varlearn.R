dlmFilterDF.var <- function (y, mod, simplify = FALSE, bloques, descuento) 
{
    ...
    # Definiciones para matriz V (univariada en este caso)
    ...
    for (i in seq(length = nrow(y))) {
      ...
      ## Cálculo de W con descuentos
      #Wliste[[i]] <- P[[i]]* ((1-DF)/DF)
      lista.mat <- lapply(1:length(bloques), 
        function(j){ 
          P[[i]][bloques[[j]],bloques[[j]]] * ((1-descuento[j])/descuento[j] )
        }
      )
      Wliste[[i]] <- as.matrix(Reduce(Matrix:::bdiag, lista.mat))
